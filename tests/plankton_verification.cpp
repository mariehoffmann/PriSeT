...
void load_primers_known(std::unordered_map<TPrimerKey, std::string, hash_pp> & pairs_known)
{
    ...
    std::cout << "dna_encoder_with_lbit(TAAGGGATCAAAGACAACCAGA) = " << kmerID2str(dna_encoder_with_lbit("TAAGGGATCAAAGACAACCAGA")) << std::endl;
...
{
    pairs.clear();
    for (uint64_t i = 4; i < 5; ++i) //references.size(); ++i)
    {
        std::cout << "STATUS: current referece i = " << i << "/" << references.size() << std::endl;
        ...
        for (uint64_t r_fwd = 1; r_fwd < r1s.rank(reference.size()); ++r_fwd)
        {
            ...
            if (dna_decoder(kmerID_fwd).find("CAGCAGCCGCGGTAATTCC"))
                std::cout << "EUK14 fwd contained in kmerID_fwd = " << kmerID2str(kmerID_fwd) << std::endl;
            ...
            for (uint64_t r_rev = r1s.rank(w_begin) + 1; r_rev <= r1s.rank(w_end); ++r_rev)
            {
                ...
                while ((((mask_fwd - 1) << 1) & kmerID_fwd) >> 54)
                {
                    ...
                    {
                        if (mask_fwd == (1ULL << 60) && dna_decoder(kmerID_fwd).find("CAGCAGCCGCGGTAATTCC"))
                            std::cout << "EUK fwd handled ... " << std::endl;
                        TKmerID const kmerID_rev = kmerIDs.at(i).at(r_rev - 1);
                        uint64_t mask_rev = ONE_LSHIFT_63;
                        if (dna_decoder(kmerID_rev).find("CAGCAGCCGCGGTAATTCC"))
                            std::cout << "EUK14 rev contained in kmerID_fwd = " << kmerID2str(kmerID_rev) << std::endl;
                        ...
                        while ((((mask_rev - 1) << 1) & kmerID_rev) >> 54)
                        {
                            ...
                            {
                                // CAGCAGCCGCGGTAATTCC"), dna_encoder_with_lbit("GCTTAATTTGACTCAACACGGG")}]
                                if (mask_fwd == (1ULL << 60) && dna_decoder(kmerID_fwd).find("CAGCAGCCGCGGTAATTCC") &&
                                    mask_rev == (1ULL << 57) && dna_decoder(kmerID_rev).find("GCTTAATTTGACTCAACACGGG"))
                                    std::cout << "EUK fwd + rev handled, " << dTm(kmerID_fwd, mask_fwd, kmerID_rev, mask_rev) << std::endl;
